...
try {
  ...
  const lintConfigFile = path.join(dir, 'oxlint.config.mjs')
  const fmtConfigFile = path.join(dir, 'oxfmt.config.mjs')
  if (!fs.existsSync(lintConfigFile) || !fs.existsSync(fmtConfigFile)) {
    fail('init did not create both antfu config files')
  }
  if (!fs.readFileSync(lintConfigFile, 'utf8').includes("from 'oxlint-vue/antfu'")) {
    fail('init did not use the built-in antfu lint config')
  }
  if (!fs.readFileSync(fmtConfigFile, 'utf8').includes("from 'oxlint-vue/antfu'")) {
    fail('init did not use the built-in antfu format config')
  }
...
`)
  ...
  step('running `--fix --format-code` with the built-in preset')
  try {
