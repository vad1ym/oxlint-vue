...
import { compareCase, corpus } from './compat/runner.js'
...
test('real oxlint pipeline matches the generated reference corpus and preserves source positions', async () => {
  ...
  try {
    const cases = [...generatedCases, ...realCases, ...propCases, ...corpus.cases.filter(c => c.rule === 'vue/no-mutating-props')]
    ...
    for (const key of new Set(cases.map(c => JSON.stringify([c.rule, c.options])))) {
      ...
      for (const [index, entry] of entries.entries()) {
        ...
        files.push(filename)
        expected.set(filename, entry.intentionalDifference ? [] : compareCase(entry).expected)
